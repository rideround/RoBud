(function () {
  ...
  function renderAllPlanCards(data, state) {
    ...
    if (!container) return;
    var goldUrl = (data && data.gamepassUrlGold) || "";
    var diamondUrl = (data && data.gamepassUrlDiamond) || "";
    var discordUrl = (data && data.discordInvite) || DISCORD_INVITE_URL;
        ...
        if (current === "gold") {
          ...
          if (current === "free" && (goldUrl || discordUrl)) {
            ctaEl.innerHTML = (goldUrl ? '<a href="' + escapeAttr(goldUrl) + '" target="_blank" rel="noopener" class="plan-upgrade-single btn-robux">Gold 1 mo (2,000 R$)</a>' : "") +
              (goldUrl && discordUrl ? " " : "") +
              '<a href="' + escapeAttr(discordUrl) + '" target="_blank" rel="noopener" class="plan-upgrade-single btn-discord">Buy with USD</a>';
